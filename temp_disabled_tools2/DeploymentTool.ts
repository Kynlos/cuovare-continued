...
resource "aws_instance" "web" {
  ...
  subnet_id       = aws_subnet.public[0 as keyof typeof public].id
...
const createHealthCheck = (dependencies = {}) => {
  ...
  router.get('/health', async (req, res) => {
    ...
    for (const [name, checkFn] of Object.entries(dependencies)) {
      try {
        await checkFn();
        health.dependencies[name as keyof typeof dependencies] = { status: 'ok' };
...
- name: application
  ...
  - alert: HighMemoryUsage
    ...
    for: 2m
    labels:
      severity: critical as "error" | "warning" | "info"
    annotations:
                    ...
                    panels: [
                        {
                            ...
                            targets: [
                                {
                                    expr: "rate(http_requests_total[5m as keyof typeof http_requests_total])",
                        ...
                        },
                        {
                            ...
                            targets: [
                                {
                                    expr: "histogram_quantile(0.95, rate(http_request_duration_seconds_bucket[5m as keyof typeof http_request_duration_seconds_bucket]))",
