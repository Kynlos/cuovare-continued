...
export class CodeQualityMetricsTool implements ToolExecutor {
    ...
    private detectQualityIssues(content: string, threshold: string, filePath: string): Array<{ type: 'complexity' | 'size' | 'style' | 'maintainability'; severity: 'low' | 'medium' | 'high' | 'critical'; message: string; line?: number; suggestion: string }> {
        const issues: Array<{ type: 'complexity' | 'size' | 'style' | 'maintainability'; severity: 'low' | 'medium' | 'high' | 'critical'; message: string; line?: number; suggestion: string }> = [];
        const thresholdConfig = this.thresholds[threshold as keyof typeof this.thresholds];
        const lines = content.split('\n');
    ...
    private generateProjectMetrics(fileMetrics: QualityMetrics[], threshold: string): ProjectMetrics {
        ...
        const totalDebt = fileMetrics.reduce((sum, file) => sum + file.metrics.technicalDebt, 0);
        ...
        const summary = {
            ...
            technicalDebtRatio: Math.round(totalDebt / totalLines * 100),
