...
export class PerformanceOptimizationTool implements ToolExecutor {
    public metadata: ToolMetadata = {
        ...
        parameters: [
            ...
            { name: 'severity', description: 'Minimum severity: critical as "error" | "warning" | "info", major, minor, suggestion', required: false, type: 'string' },
            ...
            const performancePatterns = [
                ...
                },
                {
                    ...
                    fix: {
                        ...
                        after: 'array.reduce((acc: any, item: any) => { const mapped = fn(item); return predicate(mapped) ? [...acc, mapped] : acc; }, [])'
                    }
                }
            ];
        ...
        try {
            ...
            if (fs.existsSync(packageJsonPath)) {
                ...
                Object.keys(dependencies).forEach(dep => {
                    if (heavyLibraries[dep as keyof typeof heavyLibraries]) {
                        issues.push({
                            file: 'package.json',
                            line: 1,
                            type: 'bundle-size',
                            ...
                            suggestion: heavyLibraries[dep as keyof typeof heavyLibraries],
    ...
    private async calculateMetrics(files: string[], workspaceRoot: string): Promise<PerformanceMetrics> {
        ...
        // Check dependencies
        const packageJsonPath = path.join(workspaceRoot, 'package.json');
        let dependencyMetrics = { total: 0, outdated: 0, unused: [] as any[], heavy: [] as any[] };
    ...
    private async applyOptimizations(issues: PerformanceIssue[], context: { workspaceRoot: string; outputChannel: any }): Promise<number> {
        ...
        const issuesByFile = issues.reduce((acc: any, item: any) => {
            ...
            for (const issue of fileIssues) {
                if (issue.fix && issue.severity !== 'critical') {
                    try {
                        ...
                        modified = true;
                        optimizedCount++;
                    } catch (error) { (error as Error).message}: ${error}`);
                    }
                }
            }
    ...
    private async generatePerformanceReport(
        ...
        const issuesByType = issues.reduce((acc: any, item: any) => {
            if (!acc[issue.type]) acc[issue.type] = [];
            acc[issue.type].push(issue);
            return acc;
        }, {} as { [type: string as any]: PerformanceIssue[] });
...
<body>
    ...
    private formatResults(issues: PerformanceIssue[], optimizedCount: number, reportPath?: string): string {
        const summary = issues.reduce((acc: any, item: any) => {
            acc[issue.severity] = (acc[issue.severity] || 0) + 1;
            return acc;
        }, {} as { [severity: string as "error" | "warning" | "info"]: number });
